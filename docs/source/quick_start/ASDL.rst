...
Import QoreTek_FlowSiN_250_V1p0p4_Latitudeda PDK into AdvancedSDL
*******************************************

Copy the **QoreTek_FlowSiN_250_V1p0p4_Latitudeda PDK** folder to the **AdvancedSDL** installation path: ``...\AdvancedSDL\Python\Lib\site-packages``. It can also be placed in other paths that do not contain special symbols such as Chinese characters and Spaces, so that python does not fail to identify the path.
...
Locate and select the ``QoreTek_FlowSiN_250_V1p0p4_Latitudeda`` PDK folder.

.. image:: ../images/ASDL_2.png

After the import is complete, you will see the **QoreTek_FlowSiN_250_V1p0p4_Latitudeda** PDK component library on the left.
...
Enter the project name and path, select ``QoreTek_FlowSiN_250_V1p0p4_Latitudeda`` PDK in the PDK column, and click ``OK``.

.. image:: ../images/ASDL_5.png

Wait for the contents of ``Import QoreTek_FlowSiN_250_V1p0p4_Latitudeda pdk symbols succeed`` to appear in the output.
